...
class FichaIndividualCreate extends Component
{
    ...
    public function updatednumedoc1()
    {
        ...
        try {
            $urlCompleta = $urldni . $dni;

            Log::info('Iniciando consulta de DNI', [
                'dni' => $dni,
                'url' => $urlCompleta,
            ]);
            ...
            Log::info('Respuesta de la API DNI', [
            ...
            if (
                ...
                session()->flash(
                    'warning',
                    'La API no encontró datos. Respuesta: ' .
                    substr($response->body(), 0, 300)
                );
            ...
            session()->flash('success', 'DNI encontrado correctamente.');
            ...
            Log::error('Error consultando API de DNI', [
                'tipo' => get_class($e),
    ...
    public function register()
    {
        try {
            ...
            $url = env('URL_MAP') . "/servicio/wms?service=WMS&request=getMap&version=1.1.1&layers=lote,id_lote,vertice_lote,eje_via&styles=&WIDTH=1680&HEIGHT=834&SRS=EPSG%3A32719&BBOX=" . $extension[0]->extension . "&format=image/png&id=" . $ficha->id_lote;
