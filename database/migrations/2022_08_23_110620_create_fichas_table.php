...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('tf_fichas', function (Blueprint $table) {
            ...
            $table->foreign('id_declarante')->references('id_persona')->on('personas')->onDelete('cascade')->onUpdate('cascade');
            $table->date('fecha_declarante')->nullable();
            $table->string('id_supervisor', 21)->unsigned()->nullable();
            $table->foreign('id_supervisor')->references('id_persona')->on('personas')->onDelete('cascade')->onUpdate('cascade');
            $table->date('fecha_supervision')->nullable();
            $table->string('id_tecnico', 21)->unsigned()->nullable();
            $table->foreign('id_tecnico')->references('id_persona')->on('personas')->onDelete('cascade')->onUpdate('cascade');
            $table->date('fecha_levantamiento')->nullable();
            $table->string('id_verificador', 21)->unsigned()->nullable();
            $table->foreign('id_verificador')->references('id_persona')->on('personas')->onDelete('cascade')->onUpdate('cascade');
            $table->date('fecha_verificacion')->nullable();
            $table->string('nume_registro', 10)->nullable();
            $table->string('id_uni_cat', 23)->unsigned();
            $table->foreign('id_uni_cat')->references('id_uni_cat')->on('uni_cats')->onDelete('cascade')->onUpdate('cascade');
